...
kubectl create secret docker-registry gar-secret --docker-server=us-central1-docker.pkg.dev --docker-username=_json_key --docker-password="$(cat /root/myapp/gar-key.json)" --docker-email=not-used@example.com
